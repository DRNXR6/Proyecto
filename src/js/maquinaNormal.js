...
function maquina() {
    setTimeout(() => {
        ...
        for (let comb of combinaciones) {
            ...
            if (bloqueo === false && BtnTotales[comb[1]].textContent === "X" && BtnTotales[comb[2]].textContent === "X" && BtnTotales[comb[0]].textContent === "") {
                BtnTotales[comb[0]].textContent = "O";
                listaBotonesDisponibles = listaBotonesDisponibles.filter(btn => btn !== BtnTotales[comb[0]]);
                bloqueo = true;

                break;
            }
            if (bloqueo === false && BtnTotales[comb[0]].textContent === "X" && BtnTotales[comb[1]].textContent === "" && BtnTotales[comb[2]].textContent === "X") {
